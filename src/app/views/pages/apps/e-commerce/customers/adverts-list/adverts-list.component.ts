...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	get_data() {
       ...
       // this.dataSource = new MatTableDataSource([])
		    this.advertsDataService.get_adverts().subscribe((data:any) => this.ELEMENT_DATA = data.data,
	...
	ngOnInit() {
        this.get_data();
        this.advertsDataService.bClickedEvent
	...
	data_info: any[];
	edit_subject(adverts: adverts ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
		console.log('adverts ID', adverts.ser);
		this.advertsDataService.ser=adverts.ser;
		this.advertsDataService.get_adverts_with_id(adverts.ser).subscribe((data: any) => this.data_info = data.data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.data_info) {
					this.advertsDataService.title = item.title;
					this.advertsDataService.body = item.body;
					this.advertsDataService.from_emp_id = item.from_emp_id;
					this.advertsDataService.submit_date = item.submit_date;
					this.advertsDataService.state = item.state;
					this.advertsDataService.start_date = item.start_date;
					this.advertsDataService.end_date = item.end_date;

				   
				   
				};
				console.log('Component A is clicked!!', this.advertsDataService.ser);
				this.advertsDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	delete_subject(adverts: adverts) {
	
		console.log('adverts ID', adverts.ser);
		this.advertsDataService.delete_from_adverts(Number(adverts.ser)).subscribe(res => {
			this.get_data();
			alert(res.toString());
	...
	updateCheckedList(adverts) {
		 
		if (adverts.state == "true") {
			console.log("d5lt!")
			adverts.state = "1"
		}
		if (adverts.state == "false") {
			console.log("d5lt!")
			adverts.state = "0"
		}
		var val = {
			ser: Number(adverts.ser),
			state: Number(adverts.state)

		};
		console.log("val", val);
		this.advertsDataService.update_adverts_state(val).subscribe(res => {
			console.log("updated!");
