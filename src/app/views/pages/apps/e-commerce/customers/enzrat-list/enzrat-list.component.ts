...
import { enzrat,enzratMaster } from '../../../../../../enzrat.Model';
...
@Component({
	...
	selector: 'kt-enzrat-list',
	templateUrl: './enzrat-list.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class enzratListComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'ser', 'civil_id', 'student', 'alert_type','actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	get_data() {
       ...
       // this.dataSource = new MatTableDataSource([])
		    this.enzratDataService.get_enzrat().subscribe((data:any) => this.ELEMENT_DATA = data.data,
	...
	ngOnInit() {
        this.get_data();
        this.enzratDataService.bClickedEvent
	...
	edit_subject(enzrat: enzrat ) {
		...
		this.enzratDataService.ser=enzrat.ser;
		this.enzratDataService.get_enzrat_with_id(enzrat.ser).subscribe((data: any) => this.data_info = data.data,
			...
			() => {
				...
				console.log('Component A is clicked!!', this.enzratDataService.ser);
				this.enzratDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	delete_subject(enzrat: enzrat) {
...
// 	updateCheckedList(enzrat) {
		 
// 		if (enzrat.state == "true") {
// 			console.log("d5lt!")
// 			enzrat.state = "1"
// 		}
// 		if (enzrat.state == "false") {
// 			console.log("d5lt!")
// 			enzrat.state = "0"
// 		}
// 		var val = {
// 			ser: Number(enzrat.ser),
// 			state: Number(enzrat.state)

// 		};
// 		console.log("val", val);
// 		this.enzratDataService.update_enzrat_state(val).subscribe(res => {
// 			console.log("updated!");
