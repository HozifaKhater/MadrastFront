...
export class failureListComponent implements OnInit, OnDestroy {
    ...
    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'fail_id', 'civil_id','student_name', 'fail_sub', 'fail_end_year', 'fail_sit','fail_recomm', 'actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	deleteCustomers() {
		for (let i = 0; i < this.selection.selected.length; i++) {
			this.Failure_casesDataService.deleteFailure_cases(Number(this.selection.selected[i].fail_id))
	...
	ngOnInit() {
		...
		this.Failure_casesDataService.bClickedEvent
		.subscribe((data: string) => {
			this.get_failure();
		});
		this.get_failure()
