...
@Component({
	...
	selector: 'kt-absence_statistics',
	templateUrl: './absence_statistics.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class absence_statisticsListComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'absence_id', 'absence_lev', 'absence_class', 'absence_date','absence_student_name','civil_id', 'actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	get_absence(date, lev_id) {
		if (lev_id > 0)
		{
		var val = {
			absence_date:date,
			absence_lev:String(lev_id)
		}
		this.AbsenceDataService.get_absence_with_date_and_level(val).subscribe(data => this.ELEMENT_DATA = data,
			error => console.log(error),
			() => this.dataSource.data = this.ELEMENT_DATA); 
		}
	else
	{
		this.AbsenceDataService.get_absence_with_date(date).subscribe(data => this.ELEMENT_DATA = data,
			error => console.log(error),
			() => this.dataSource.data = this.ELEMENT_DATA); 
	}
	}
	...
	ngOnInit() {
		this.AbsenceDataService.cClickedEvent
		.subscribe((data: string) => {
			this.get_absence( this.AbsenceDataService.absence_date,this.AbsenceDataService.absence_lev);
		});
