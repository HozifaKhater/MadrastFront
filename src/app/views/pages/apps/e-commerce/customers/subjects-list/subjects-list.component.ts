...
export class SubjectComponent implements OnInit, OnDestroy {
	...
	deleteCustomers() {
		for (let i = 0; i < this.selection.selected.length; i++) {
			...
			this.SubjectService.deleteSubject(Number(this.selection.selected[i].subject_id))
			.subscribe(res => {
				this.get_subjects();
			})
		}
	...
	edit_subject(subjects: Subjects) {

		this.SubjectService.subject_id =subjects.subject_id;

		this.SubjectService.GetAllSubject_with_id(subjects.subject_id)
		.subscribe(data => this.subjects_info = data,
			error => console.log(),
			() => {
				for (let item of this.subjects_info) {
					...
					this.SubjectService.dep_name = item.dep_name;
					this.SubjectService.dep_id = item.dep_id;
					this.SubjectService.parent_id = item.parent_id;
				};
	...
	delete_subject(Subjects: Subjects) {
