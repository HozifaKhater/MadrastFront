...
export class student_basic_dataListComponent implements OnInit, OnDestroy {
    ...
    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'delay_id','civil_id', 'delay_date', 'time_attend','actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	deleteCustomers() {
		for (let i = 0; i < this.selection.selected.length; i++) {
			...
			this.student_basic_dataDataService.deletestudent_basic_data(Number(this.selection.selected[i].ser))
			.subscribe(res => {
				this.get_delays()
			})
	...
	ngOnInit() {
		...
		this.student_basic_dataDataService.bClickedEvent
    ...
    editCustomer(student_basic_data: student_basic_data, student_basic_dataDataService: student_basic_dataDataService ) {
        ...
        this.student_basic_dataDataService.GetAll_student_basic_data_with_id(student_basic_data.ser)
		.subscribe(data => this.Delays_info = data,
