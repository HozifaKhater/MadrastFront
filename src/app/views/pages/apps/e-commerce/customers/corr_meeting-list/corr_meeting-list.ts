...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	get_corr_meet() {
		...
		this.Corr_meetingDataService.GetAllCorr_meeting()
	...
	deleteCustomers() {
		for (let i = 0; i < this.selection.selected.length; i++) {
			this.Corr_meetingDataService.deleteCorr_meeting(Number(this.selection.selected[i].corr_meet_id))
			.subscribe(res => {
				this.get_corr_meet()
			})
		}
	...
	ngOnInit() {
		...
		this.Corr_meetingDataService.bClickedEvent
	...
	Observ_info: any [];
	editCustomer(customer: Corr_meeting ) {
