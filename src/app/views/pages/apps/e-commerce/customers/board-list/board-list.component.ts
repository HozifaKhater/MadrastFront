...
export class boardListComponent implements OnInit, OnDestroy {
    ...
    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'ser', 'dep_name', 'strength', 'weakness','chances','risks','actions'];
...
private router: Router, private user_privDataService: user_privDataService,
	...
	get_data() {
       ...
       // this.dataSource = new MatTableDataSource([])
		    this.boardDataService.get_board().subscribe((data:any) => this.ELEMENT_DATA = data.data,
...
deleteCustomers() {
		for (let i = 0; i < this.selection.selected.length; i++) {
			...
			this.boardDataService.delete_from_board(Number(this.selection.selected[i].board_id)).subscribe(res => {
				this.get_data()
	...
	ngOnInit() {
        ...
        this.get_data();
        this.boardDataService.bClickedEvent
	...
	edit_subject(board: board ) {
		...
		this.boardDataService.board_id=board.board_id
		this.boardDataService.get_board_with_id(board.board_id).subscribe((data: any) => this.data_info = data.data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.data_info) {
					
					this.boardDataService.board_name = item.board_name;
					this.boardDataService.board_type_id = item.board_type_id;
					this.boardDataService.lev_id = item.lev_id;
					this.boardDataService.lev_name = item.lev_name;
					this.boardDataService.student_name = item.student_name;
					this.boardDataService.student_id = item.student_id;
					this.boardDataService.rank_id = item.rank_id;
					this.boardDataService.rank_name = item.rank_name;
					this.boardDataService.parent_name = item.parent_name;
					this.boardDataService.parent_job = item.parent_job;
					this.boardDataService.address = item.address;
					this.boardDataService.mobile = item.mobile;
					this.boardDataService.mobile2 = item.mobile2;
					this.boardDataService.dep_name = item.dep_name;
					this.boardDataService.dep_id = item.dep_id;
					this.boardDataService.job_name = item.job_name;
					this.boardDataService.job_id = item.job_id;
					this.boardDataService.emp_name = item.emp_name;
					this.boardDataService.emp_id = item.emp_id;
				   
				   
				   
				};
				console.log('Component A is clicked!!', this.boardDataService.board_id);
				this.boardDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	delete_subject(board: board) {
	
		console.log('board ID', board.board_id);
		this.boardDataService.delete_from_board(Number(board.board_id)).subscribe(res => {
			this.get_data();
			alert(res.toString());
