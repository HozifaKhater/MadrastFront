...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	get_corridros() {
		this.corridorsDataService.GetAllCorridors().subscribe(data => this.ELEMENT_DATA = data,
			error => console.log(),
	...
	deleteCustomers() {
		for (let i = 0; i < this.selection.selected.length; i++) {
			this.corridorsDataService.deleteCorridors
			(Number(this.selection.selected[i].corridor_id))
			.subscribe(res => {
				this.get_corridros()
			})
		}
	...
	ngOnInit() {
		...
		this.corridorsDataService.bClickedEvent
 ...
 /**
	...
	edit_corridor(corridor: corridor, corridorsDataService: corridorsDataService ) {
		...
		this.corridorsDataService.GetAllCorridors_with_id(corridor.corridor_id)
		.subscribe(data => this.corridor_info = data,
			error => console.log(),
			() => {
	...
	delete_corridor(corridor: corridor ) {
	
		this.corridorsDataService.deleteCorridors(Number(corridor.corridor_id))
		.subscribe(res => {
			this.get_corridros();
