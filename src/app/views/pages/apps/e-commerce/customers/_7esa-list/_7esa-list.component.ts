...
@Component({
	...
	selector: 'kt-_7esa-list',
	templateUrl: './_7esa-list.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class _7esaListComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'ser', 'start_time', 'end_time', 'actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	constructor(
		...
		private _7esaDataService: _7esa_defDataService
	...
	get_data() {
       ...
       // this.dataSource = new MatTableDataSource([])
		    this._7esaDataService.get_7esa_def().subscribe((data:any) => this.ELEMENT_DATA = data.data,
	...
	ngOnInit() {
        this.get_data();
        this._7esaDataService.bClickedEvent
	...
	data_info: any[];
	edit_subject(_7esa: _7esa ) {

		console.log('_7esa ID', _7esa.ser);
		this._7esaDataService.ser=_7esa.ser;
		this._7esaDataService.get_7esa_def_with_id(_7esa.ser).subscribe((data: any) => this.data_info = data.data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.data_info) {
					this._7esaDataService.start_time = item.start_time;
					this._7esaDataService.end_time = item.end_time; 
				};
				console.log('Component A is clicked!!', this._7esaDataService.ser);
				this._7esaDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	delete_subject(_7esa: _7esa) {
	
		console.log('_7esa ID', _7esa.ser);
		this._7esaDataService.delete_from_7esa_def(Number(_7esa.ser)).subscribe(res => {
			this.get_data();
			alert(res.toString());
