...
import { behaviours_status,behaviours_statusMaster } from '../../../../../../behaviours_status.Model';
...
@Component({
	...
	selector: 'kt-behaviours_status-list1',
	templateUrl: './behaviours_status-list.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class behaviours_statusListComponent1 implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'id', 'notes','civil_id','student_name', 'reasons','actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	dataSource: any;
		...
		behaviours_status: behaviours_status[];
	...
	get_data() {
		...
		this.behaviours_statusDataService.get_behaviours_status()
	...
	deleteCustomers() {
		for (let i = 0; i < this.selection.selected.length; i++) {
			this.behaviours_statusDataService.delete_from_behaviours_status
...
*/
	...
	data_info: any[];
	editCustomer(behaviours_status: behaviours_status ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
		console.log('behaviours_status ID', behaviours_status.id);
		this.behaviours_statusDataService.id=behaviours_status.id
		this.behaviours_statusDataService.get_behaviours_status_with_id(behaviours_status.id).subscribe((data: any) => this.data_info = data.data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.data_info) {

					this.behaviours_statusDataService.status_type = item.status_type;
					this.behaviours_statusDataService.notes = item.notes;
					this.behaviours_statusDataService.reasons = item.reasons;
					this.behaviours_statusDataService.lev_id = item.lev_id;
					this.behaviours_statusDataService.class_id = item.class_id;
					this.behaviours_statusDataService.student_id = item.student_id;
				   
				};
				console.log('Component A is clicked!!', this.behaviours_statusDataService.id);
				this.behaviours_statusDataService.AClicked('Component A is clicked!!');
				this.behaviours_statusDataService.get_details_CClicked('Component A is clicked!!');
			}
		);
		

	}
	deleteCustomer(behaviours_status: behaviours_status) {
	
		console.log('behaviours_status ID', behaviours_status.id);
		this.behaviours_statusDataService.delete_from_behaviours_status(Number(behaviours_status.id)).subscribe(res => {
			this.get_data();
