...
@Component({
	...
	selector: 'kt-month_value-list',
	templateUrl: './month_value-list.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class month_valueListComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'ser', 'title', 'body', 'state','actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	get_data() {
       ...
       // this.dataSource = new MatTableDataSource([])
		    this.month_valueDataService.get_month_value().subscribe((data:any) => this.ELEMENT_DATA = data.data,
	...
	ngOnInit() {
        this.get_data();
        this.month_valueDataService.bClickedEvent
	...
	data_info: any[];
	edit_subject(month_value: month_value ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
		console.log('month_value ID', month_value.ser);
		this.month_valueDataService.ser=month_value.ser;
		this.month_valueDataService.get_month_value_with_id(month_value.ser).subscribe((data: any) => this.data_info = data.data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.data_info) {
					this.month_valueDataService.title = item.title;
					this.month_valueDataService.body = item.body;
					this.month_valueDataService.from_emp_id = item.from_emp_id;
					this.month_valueDataService.submit_date = item.submit_date;
					this.month_valueDataService.state = item.state;
					this.month_valueDataService.month = item.month;
					this.month_valueDataService.year = item.year;
				   
				   
				   
				};
				console.log('Component A is clicked!!', this.month_valueDataService.ser);
				this.month_valueDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	delete_subject(month_value: month_value) {
	
		console.log('month_value ID', month_value.ser);
		this.month_valueDataService.delete_from_month_value(Number(month_value.ser)).subscribe(res => {
			this.get_data();
			alert(res.toString());
	...
	updateCheckedList(month_value) {
		 
		if (month_value.state == "true") {
			console.log("d5lt!")
			month_value.state = "1"
		}
		if (month_value.state == "false") {
			console.log("d5lt!")
			month_value.state = "0"
		}
		var val = {
			ser: Number(month_value.ser),
			state: Number(month_value.state)

		};
		console.log("val", val);
		this.month_valueDataService.update_month_value_state(val).subscribe(res => {
			console.log("updated!");
