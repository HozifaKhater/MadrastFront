...
export class BadStudentsCardsListComponent implements OnInit, OnDestroy {
    ...
    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'student_card_id','civil_id', 'student_name', 'bad_da3f', 'bad_da3f_reasons', 'bad_result', 'actions'];
	...
	editCustomer(customer: Good_bad_students_card, Good_bad_students_cardDataService: Good_bad_students_cardDataService ) {
		...
		this.Good_bad_students_cardDataService.GetAllgoodbadstudents_with_id(customer.student_card_id).subscribe(data => this.goodbadstudents_info = data,
			...
			() => {
				...
				console.log('Component A is clicked!!', this.Good_bad_students_cardDataService.student_card_id);
