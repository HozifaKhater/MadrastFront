...
@Component({
	...
	selector: 'kt-teacher_opinion_visit-list',
	templateUrl: './teacher_opinion_visit-list.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class teacher_opinion_visitListComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select','ser', 'is_agree', 'notes', 'actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	dataSource: any;
		...
		teacher_opinion_visit: teacher_opinion_visit[];
	...
	get_data() {
       ...
       // this.dataSource = new MatTableDataSource([])
		    this.teacher_opinion_visitDataService.get_teacher_opinion_visit().subscribe((data:any) => this.ELEMENT_DATA = data.data,
	...
	deleteCustomers() {
		for (let i = 0; i < this.selection.selected.length; i++) {
			...
			this.teacher_opinion_visitDataService.delete_from_teacher_opinion_visit
				(Number(this.selection.selected[i].ser)).subscribe(res => {
	...
	ngOnInit() {
        ...
        this.get_data();
        this.teacher_opinion_visitDataService.bClickedEvent
...
*/
	...
	edit_subject(teacher_opinion_visit: teacher_opinion_visit ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
		console.log('teacher_opinion_visit ID', teacher_opinion_visit.ser);
		this.teacher_opinion_visitDataService.get_teacher_opinion_visit_id(teacher_opinion_visit.ser)
		.subscribe((data: any) => this.data_info = data.data,
		//this.teacher_opinion_visitDataService.get_teacher_opinion_visit_id(teacher_opinion_visit.ser).subscribe(data => this.data_info = data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.data_info) {
					this.teacher_opinion_visitDataService.ser = item.ser;
					this.teacher_opinion_visitDataService.takeem_id	=	item.takeem_id	;
					this.teacher_opinion_visitDataService.emp_id	=	item.emp_id	;
					this.teacher_opinion_visitDataService.is_agree	=	item.is_agree	;
					this.teacher_opinion_visitDataService.notes	=	item.notes	;
				   
				};

				console.log('Component A is clicked!!', this.teacher_opinion_visitDataService.ser);
				this.teacher_opinion_visitDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	delete_subject(teacher_opinion_visit: teacher_opinion_visit) {
	
		console.log('teacher_opinion_visit ID', teacher_opinion_visit.ser);
		this.teacher_opinion_visitDataService.delete_from_teacher_opinion_visit(Number(teacher_opinion_visit.ser)).subscribe(res => {
			this.get_data();
			alert("Deleted Succesfullu");
