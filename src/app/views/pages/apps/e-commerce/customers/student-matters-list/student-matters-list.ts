...
@Component({
	...
	selector: 'kt-matters-list',
	templateUrl: './student-matters-list.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class studentmattersListComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['select', 'id', 'level_name', 'class_name', 'topic','ntoes', 'actions'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	student_matters: student_matters[];
	...
	get_matters() {
		...
		this.student_mattersDataService.GetAllstudent_matters().subscribe(data => this.ELEMENT_DATA = data,
			error => console.log(),
	...
	deleteCustomers() {
		for (let i = 0; i < this.selection.selected.length; i++) {
			this.student_mattersDataService.deletestudent_matters
			(Number(this.selection.selected[i].id))
	...
	ngOnInit() {
		...
		this.student_mattersDataService.bClickedEvent
			.subscribe((data: string) => {
				this.get_matters();
			});
		this.get_matters()
	...
	matters_info: any [];
	editCustomer(customer: student_matters, student_mattersDataService: student_mattersDataService ) {

		this.student_mattersDataService.GetAllstudent_matters_with_id(customer.id).subscribe(data => this.matters_info = data,
			error => console.log(),
			() => {
				for (let item of this.matters_info) {

					this.student_mattersDataService.id = Number(item.id);
					
					this.student_mattersDataService.level_id = item.level_id;
					this.student_mattersDataService.level_name = item.level_name;
					this.student_mattersDataService.class_id = item.class_id;
					this.student_mattersDataService.class_name = item.class_name;

					this.student_mattersDataService.note_date = item.note_date;
					this.student_mattersDataService.topic = item.topic;
					this.student_mattersDataService.ntoes = item.ntoes;
					this.student_mattersDataService.student_id = item.student_id;
					this.student_mattersDataService.student_name = item.student_name;
		
				};
				this.student_mattersDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	deleteCustomer(customer: student_matters, student_mattersDataService: student_mattersDataService) {
	
		this.student_mattersDataService.deletestudent_matters(Number(customer.id)).subscribe(res => {
			this.get_matters();
