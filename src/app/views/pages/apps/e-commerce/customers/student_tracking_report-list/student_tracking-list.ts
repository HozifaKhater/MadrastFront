...
@Component({
	...
	selector: 'kt-student_tracking_report-list',
	templateUrl: './student_tracking-list.html',
	changeDetection: ChangeDetectionStrategy.OnPush


})
export class student_tracking_reportListComponent implements OnInit, OnDestroy {
	// Table fields

	Element1: [{ id: "1" },
		{ id: "2" }];
	displayedColumns = ['select', 'ser', 'student_name', 'behavior','book','practice'];
	...
	filterType: string = '';
	// Selection
	selection = new SelectionModel<student_tracking>(true, []);
    ...
    get_data(class_id,date) {
		var val={
			class_id: class_id,
			date:date
		}
        this.student_trackingDataService.get_student_tracking_with_date_and_classid(val).subscribe((data: any) => {console.log("datarep",data.data);this.ELEMENT_DATA = data.data},
            error => console.log(error),
            () => this.dataSource.data = this.ELEMENT_DATA);
        console.log("zzzrep", this.ELEMENT_DATA);}
	/**
	 ...
	 * On init
	 */
	data_Change(event,student_tracking) {
		console.log("changed",this.dataSource.data,student_tracking)
		of(event).pipe(debounceTime(2000)).subscribe(val => {
			
				console.log("forloop")
				var val2 = {
					
					term_id: this.student_trackingDataService.term_id,
					subject_id: this.student_trackingDataService.subject_id,
					level_id: this.student_trackingDataService.level_id,
					class_id: this.student_trackingDataService.class_id,
					date: this.student_trackingDataService.date,
					student_id:String(student_tracking.student_id),
					behavior:student_tracking.behavior,
					book:student_tracking.book,
					practice:student_tracking.practice
							};
				this.student_trackingDataService.save_update_student_tracking(val2).subscribe(res => {  console.log("val2", val2) })
	...
	ngOnInit() {
        this.student_trackingDataService.cClickedEvent
            .subscribe((data: string) => {
                this.get_data(this.student_trackingDataService.class_id,this.student_trackingDataService.date)
                console.log("asasd")
            });
	 ...
	 */
	masterToggle() {
		if (this.selection.selected.length === this.student_tracking.length) {
			this.selection.clear();
		} else {
			this.student_tracking.forEach(row => this.selection.select(row));
		}
	}
