...
import htmlcan from 'html2canvas'
// NGRX
...
@Component({
	...
	selector: 'kt-show-ezons-list',
	templateUrl: './show_ezons-list.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class ShowEzonListComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['ezn_id','emp_name', 'emp_id', 'ezn_date', 'time_from', 'time_to','ezn_state'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	downloadAsPDF() {
		var htm = document.getElementById("pdfTable");
		htmlcan(htm).then((canvas)=>{
			var data = canvas.toDataURL('image/png');
			var doc = new jsPDF.jsPDF();
			var hei = canvas.height * 208 / canvas.width;
			doc.addImage(data,0,0,208,hei);
			doc.save('data.pdf');
		})
    }
	decoded:any;
	...
	get_ezons() {
		...
		this.EmployeeDataService.GetAllEmployee_with_id(this.decoded.id).subscribe(data => this.Employee = data,
			...
			() => {
			  if( (this.Employee[0].emp_pos_id == String(37))  || (this.Employee[0].emp_pos_id == String(41)))
			  {
				
				this.EzonDataService.GetAllEzon_with_emp_id(this.Employee[0].emp_id).subscribe(data => this.ELEMENT_DATA = data,
					error => console.log(error),
					() => this.dataSource.data = this.ELEMENT_DATA
				); 
			  }
			  else if(this.Employee[0].emp_pos_id == String(38)){

				this.EzonDataService.get_absent_premit_ezoon_with_dep_id(this.Employee[0].emp_dep_id).subscribe(data => this.ELEMENT_DATA = data,
					error => console.log(error),
					() => this.dataSource.data = this.ELEMENT_DATA
				); 
			  }
			  ...
			  {
					this.EzonDataService.GetAllEzon().subscribe(data => this.ELEMENT_DATA = data,
			error => console.log(error),
			() => this.dataSource.data = this.ELEMENT_DATA
		); 
			  }
