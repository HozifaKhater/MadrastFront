...
// Services and Models
import { CustomerModel, CustomersPageRequested, ManyCustomersDeleted, CustomersStatusUpdated } 
from '../../../../core/e-commerce';
// Components
import { calling_parentsDataService } from '../../../../Services/calling_parentsDataService';
import { calling_parents,calling_parentsMaster } from '../../../../calling_parentsMaster.Model';
...
@Component({
	...
	selector: 'kt-calling_parents-list',
	templateUrl: './calling_parents-list.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush
	
	/*,providers: [DepartmentDataService]*/
})
export class calling_parentsListComponent implements OnInit, OnDestroy {
	// Table fields


    Element1: [{ id: "1" },
        { id: "2" }];
	displayedColumns = ['ser', 'student_name', 'date'];
...
/*	dataSource: [{ "dep_id": 1, "dep_name": "main dep", "dep_desc": "asdasd", "dep_supervisor_id": 0, "dep_supervisor_name": "1", "parent_id": 1 }];*/
	...
	dataSource: any;
		...
		calling_parents: calling_parents[];
	...
	get_data() {
       ...
       // this.dataSource = new MatTableDataSource([])
		    this.calling_parentsDataService.get_calling_parents().subscribe((data:any) => this.ELEMENT_DATA = data.data,
		    	error => console.log(error),
                () => {
					this.dataSource.data = this.ELEMENT_DATA;
					console.log("parent " , this.ELEMENT_DATA );
				});
    }
	/**
	...
	ngOnInit() {
        this.get_data();
        this.calling_parentsDataService.bClickedEvent
	...
	data_info: any[];
	edit_subject(calling_parents: calling_parents ) {

		//this.DepartmentService.data = Number(customer.dep_id)
		//console.log('CUSTOMER ID', Number(customer.dep_id));
		console.log('calling_parents ID', calling_parents.ser);
		this.calling_parentsDataService.get_calling_parents_with_id(calling_parents.ser).subscribe((data: any) => this.data_info = data.data,
			error => console.log("errorrrrrrrrrrr"),
			() => {
				for (let item of this.data_info) {
					this.calling_parentsDataService.date = item.date;
					this.calling_parentsDataService.lev_name = item.lev_name;
					this.calling_parentsDataService.lev_id = item.lev_id;
					this.calling_parentsDataService.class_name = item.class_name;
					this.calling_parentsDataService.class_id = item.class_id;
					this.calling_parentsDataService.meeting_date = item.meeting_date;
					this.calling_parentsDataService.student_name = item.student_name;
					this.calling_parentsDataService.student_id = item.student_id;
					this.calling_parentsDataService.meeting_side_name = item.meeting_side_name;
					this.calling_parentsDataService.meeting_side_id = item.meeting_side_id;

				   
				   
				};
				console.log('Component A is clicked!!', this.calling_parentsDataService.ser);
				this.calling_parentsDataService.AClicked('Component A is clicked!!');
			}
		);
		

	}
	delete_subject(calling_parents: calling_parents) {
	
		console.log('calling_parents ID', calling_parents.ser);
		this.calling_parentsDataService.delete_from_calling_parents(Number(calling_parents.ser)).subscribe(res => {
			this.get_data();
			alert(res.toString());
