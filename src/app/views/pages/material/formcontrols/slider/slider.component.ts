...
export class SliderComponent implements OnInit {
  ...
  /*  dataSource2 = new MatTableDataSource<PeriodicElement>(ELEMENT_DATA2);*/
    ...
    jobs: MasterJob[];
    form1: FormGroup;
    constructor(private modalService: NgbModal,public _fb: FormBuilder, private http: HttpClient, private MasterJobsDataService: MasterJobsDataService, private EmployeeDataService: EmployeeDataService, private user_privDataService: user_privDataService) {
    ...
    job_selection(event) {
        ...
        this.user_privDataService.job_id=event.source.value.job_id;
       
        console.log("devent.source.value.job_i", event.source.value.job_id)
    ...
    //       error => console.log(error),
    //       () => {
    //         console.log(" this.privs", this.privs)
    //         this.newArray = [];
    //         for (var i = 0; i < this.privs.length; i++) {
    //           this.newArray.push(this.privs[i]);
    //         }
    //         this.user_privDataService.GetAlluser_privs_with_emp_id(this.user_privDataService.emp_id).subscribe(data => this.privs_edit = data,
    //           error => console.log(error),
    //           () => {
    ...
    //               for (var j = 0; j < this.privs_edit.length; j++) {
    //                // console.log("privsss", this.privs[i].priv_id,Number(this.privs_edit[j].priv_name))
                   
    //                 if (this.privs[i].priv_id == Number(this.privs_edit[j].priv_name)) {
    //                   console.log("privsss_afterif")
    //                   ismatch = true;
    
    //                   this.privs[i].checked = true;
    //                   this.newArray.push(this.privs[i]);
    //                   break;
    //                 }
    //               }
    ...
    //               if (!ismatch) {
    //                 this.privs[i].checked = false;
    //                 this.newArray.push(this.privs[i]);
    //               } 
    //               console.log("this.n",this.newArray)
    //             }; });
    //   })
    ...
    //   forkJoin([
    //     this.EmployeeDataService.GetAllEmployee_of_job(event.source.value.job_id),
    //     this.MasterJobsDataService.GetAllprivs_with_job_id(event.source.value.job_id),
    //     this.user_privDataService.GetAlluser_privs_with_emp_id(this.user_privDataService.emp_id)
    //   ]).subscribe(
    //     ([employees, privs, privs_edit]) => {
    //       this.employees = employees;
    //       this.privs = privs;
    //       this.privs_edit = privs_edit;
    ...
    //   this.EmployeeDataService.GetAllEmployee_of_job(event.source.value.job_id).subscribe(
    //     employees => {
    //       this.employees = employees;
    //       this.MasterJobsDataService.GetAllprivs_with_job_id(event.source.value.job_id).subscribe(
    //         privs => {
    //           this.privs = privs;
    //           this.user_privDataService.GetAlluser_privs_with_emp_id(this.user_privDataService.emp_id).subscribe(
    //             privs_edit => {
    //               this.privs_edit = privs_edit;
    ...
    emp_selection(event) {
      ...
      this.user_privDataService.emp_id=event.source.value.emp_id;
      this.user_privDataService.f_bind_tableClicked("");
    }
    ...
    add_users_priv() {
        ...
        if (this.form1.invalid) {
            ...
            this.user_privDataService.BClicked("");
    ...
    ngOnInit() {
        ...
        this.user_privDataService.aClickedEvent
            .subscribe((data: string) => {
                ...
                var selected_value_job = this.user_privDataService.job_id;
                        this.selectedjob = this.jobs[this.jobs.findIndex(function (el) {
                            return String(el.job_id) == selected_value_job
                        })];
